...
from .forms import AddPostForm, SignUpForm, SignInForm, AboutUserForm, RoomForm, CheckAvailForm, ReserveForm, PaymentForm
...
def payment():
    ...
    if request.method == "POST":
        pd = Payment(PaymentForm.room_num, PaymentForm.cardname.data, PaymentForm.cardnumber.data)      
        db.session.add(pd)
        db.session.commit()
        return render_template('payment.html', paymentform=paymentform)
    return redirect(url_for('index'))
